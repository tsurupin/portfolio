...
RSpec.describe Cms::Api::V1::AuthorsController, type: :request do
  describe 'CMS Author API' do
    ...
    describe 'GET /cms/api/v1/authors' do
    ...
    describe 'PATCH /cms/api/v1/authors' do
      ...
      context 'all the needed parameter are sent' do
        ...
        it 'updates the author account and returns status 200' do
          expect(Author.last.social_accounts.map(&:account_type)).to eq ['facebook', 'github']
          expect(response.status).to eq 200
        end
      end

      context 'the params sent has wrong info' do
        let(:params) do
          {
            'author' => {
              'name' => nil,
        ...
        let(:result) { { 'errorMessage' => "Name can't be blankValidation failed: Name can't be blank" }}
